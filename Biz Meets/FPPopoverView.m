...
- (id)initWithFrame:(CGRect)frame
{
    ...
    if (self) {
        ...
        _titleLabel.font = [UIFont fontWithName:@"Helvetica" size:15];
...
-(CGGradientRef)newGradient
{
    ...
    else if(self.tint == FPPopoverLightGrayTint)
    {
        if(_arrowDirection == FPPopoverArrowDirectionUp)
        {
            colors[0] = 0.5686; colors[1] = 0.5843; colors[2] = 0.5922;
            colors[4] = 0.5686; colors[5] = 0.5843; colors[6] = 0.5922;
            colors[3] = colors[7] = 1.0;
        }
        else
        {
...
-(void)setupViews
{
    ...
    if(_arrowDirection == FPPopoverArrowDirectionUp)
    {
		...
		if (self.title==nil || self.title.length==0) {
			contentRect.origin = CGPointMake(10, 21);
